		   ...
		   uint64_t src1, uint64_t src2, uint64_t src3, uint64_t dst)
{
	  ...
	  if (prediction_result)
	    {
	      if (U->predstride)
		if (SafeStride < (1 << 15) - 1)
		  SafeStride += 4 * (1 + (insn_class_uint8 == loadInstClass));
	    }
	  else
	    {
	      if (U->predvtage)
  ...
  bool isCondBr = insn_class_uint8 == condBranchInstClass;
  bool isUnCondBr = insn_class_uint8 == uncondIndirectBranchInstClass
    || insn_class_uint8 == uncondDirectBranchInstClass;
//path history 
